...
//		self.alertView.clipsToBounds = YES;
		...
		if (cancelTitle) {
			...
			[self addButtonWithTitle:NSLocalizedString(@"好的", nil)];
        }
...
{
	[sender setBackgroundColor:[UIColor colorWithWhite:0.937 alpha:1.000]];
}
...
- (CALayer *)lineLayer
{
	...
	lineLayer.backgroundColor = GlobalPink.CGColor;
	return lineLayer;
}
...
{
	return [[self class] showAlertWithTitle:title message:nil cancelTitle:NSLocalizedString(@"好的", nil) completion:nil];
}

+ (instancetype)showAlertWithTitle:(NSString *)title
						   message:(NSString *)message
{
	return [[self class] showAlertWithTitle:title message:message cancelTitle:NSLocalizedString(@"好的", nil) completion:nil];
}
...
+ (instancetype)showAlertWithTitle:(NSString *)title
						...
						completion:(PXAlertViewCompletionBlock)completion
{
	return [[self class] showAlertWithTitle:title message:message cancelTitle:NSLocalizedString(@"好的", nil) completion:completion];
}
...
- (NSInteger)addButtonWithTitle:(NSString *)title
{
	...
	if (!self.cancelButton) {
        ...
        [self.cancelButton setTitleColor:GlobalPink forState:UIControlStateNormal];
        [self.cancelButton setTitleColor:GlobalPink forState:UIControlStateHighlighted];
...
//		self.cancelButton.titleLabel.font = [UIFont systemFontOfSize:17];
        [self.cancelButton setTitleColor:[NLUtility colorWithHexString:@"999999"] forState:UIControlStateNormal];
        [self.cancelButton setTitleColor:[NLUtility colorWithHexString:@"999999"] forState:UIControlStateHighlighted];
	}
    [self.otherButton setTitleColor:GlobalPink forState:UIControlStateNormal];
    [self.otherButton setTitleColor:GlobalPink forState:UIControlStateHighlighted];
    [self.otherButton.titleLabel setFont:[UIFont systemFontOfSize:14]];
